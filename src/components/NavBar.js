...
// TODO: Convert the logo png to svg and use fill svg attribute to change color from light to dark. (potentially make other icons svg)
import { ReactComponent as BugsLogo } from "../assets/logo/bugs.svg";
...
const LogoImage = styled(BugsLogo)`
  ...
  fill: ${({ light }) => (light === "false" ? "#FEFAFA" : "#410092")};
...
const WideNavBar = ({ light, current, setPath }) => {
  return (
    <>
      <HeaderNavWrapper>
        <NavList>
    ...
    <div>
      <Header>
        <HeaderWrapper open={open}>
          <ContentContainer>
            <Content>
              <LeftContentContainer>{logo}</LeftContentContainer>
...
const NavBar = ({ theme }) => {
  ...
  const logo = (
    <ImageContainer onClick={setPath("")}>
      <LogoImage light={light.toString()} title="Bugs Logo" />
    </ImageContainer>
  );
  ...
  const menu =
    width <= CUTOFF ? <Menu open={open} setOpen={setOpen} /> : <div />;
